...
### 7. Web Interface and Integration
...
2. Internal context updated and encrypted before saving
3. Conversation analyzed for significance (Call 2)
4. If significant:
...
2. Retrieve user's profile from Firestore (includes encrypted `context`)
3. Decrypt user's `context` field (internal TTM/CBT notes) using Cloud KMS
4. Retrieve user's `current_stage` from profile
5. Generate query embedding (768-dim vector)
6. Query Vertex AI Vector Search with user namespace filter
7. Receive top-k similar memory IDs and distances
8. Batch-fetch corresponding documents from Firestore
9. Decrypt conversation summaries using Cloud KMS
10. Add temporal context ("2 days ago")
11. Include all context (decrypted internal notes, memories, stage) in TTM/CBT prompt for AI (Call 1)
...
- Internal therapeutic context (`context` field) is fully encrypted to protect the bot's clinical notes about the user's therapeutic progress and observations
- During retrieval: Load profile → Decrypt context → Query Vector Search → Get memory IDs → Fetch from Firestore → Decrypt summaries
...
- `POST /dialogflow-webhook` - Core Conversation Flow:
  ...
  - Loads and decrypts user profile (including internal `context`)
  - Retrieves `current_stage` and memories (via Vector Search + Firestore)
  - Performs **Call 1 (Main Prompt)** to get the empathetic `reply_text`, `new_stage`, and `updated_context`
  - Encrypts and saves the `updated_context` and `new_stage` to the user's profile
...
- `POST /reset_instructions` - Clear Custom Instructions:
  ...
  - Clears the encrypted `context` field (internal therapeutic notes)
...
- User's `current_stage` and decrypted `context` fed into TTM/CBT prompt (Call 1)
- Response (`reply_text` + `new_stage` + `updated_context`) generated and parsed
- Updated context encrypted and saved to profile
